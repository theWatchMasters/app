...
export function MFARegisterForm() {
    ...
    React.useEffect(() => {
        (async () => {
            const response = await authFetch(session, API_BASE_URL + "mfa/register", { method: "POST" });
            if (!response) {
                return;
            }
        ...
        <Card className="border-border/0 sm:border-border shadow-none sm:shadow-sm sm:shadow-black/5 w-full">
            <CardHeader>
                ...
                <CardDescription className="text-center sm:text-left">
                    {isRequestingCode ? <Text>
                        {t("mfa_register_code.headings.text2")}
                    </Text> : <Trans i18nKey='mfa_register.headings.text2'>
                        Click the button below to enroll in 2FA with your authenticator app. Alternatively, you can scan the QR code
            ...
            {isRequestingCode ?
                                ...
                                <Controller
                                    ...
                                    render={({ field: { onChange, onBlur, value } }) => (
                                        <View className="gap-1.5">
                                            <Label htmlFor="code">{t('mfa_register_code.headings.code')}</Label>
                                            <Input
                                                id="code"
                                    ...
                                    name="code"
                                />
                                <Button onPress={() => setRequestingCode(false)} variant={"outline"}><Text>Back</Text></Button>
                            </View>
                        </CardContent>
                    )}
                />
                ...
                : <CardContent className="gap-6 items-center">
                    <Button disabled={data === null} onPress={async () => await Linking.openURL(data?.url || "")}>
                    ...
                    <Text className='font-bold text-xl'>{t("mfa_register.headings.other_device")}</Text>
                    <View className="gap-6">
                        {data ? <Image src={data.qrCode} className='w-[50%] aspect-square' /> : <Skeleton className='w-[50%] aspect-square' />}
                    </View>
                    <Button onPress={() => setRequestingCode(true)}><Text>{t("mfa_register.headings.continue")}</Text></Button>
                </CardContent>}
        </Card >
    );
}
